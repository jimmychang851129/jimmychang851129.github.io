...
用於class, struct建立時預先跑的一段function來初始化class, struct(通常是初始化變數值). 基本上struct跟class是一樣的東西, 可以用variable, member function, contructor, destructor，但struct所有member function, variable都是public, 沒有private
...
使用情境有5種(以下5種只能用initialized list來init):<br />
1. init const variable
2. init reference class member
3. init object that doesn't have default constructor
4. 如果此class是derived class, 要init base class member時
5. 參數名稱跟class member的名稱一樣時

此外還有performane issue:<br />
使用initialize list就是class member物件會直接使用copy constructor把參數的物件init到這個class member<br />
如果使用assignment operater, 那就是class member物件會先用default constructor建立, 再進行assignment operator把物件複製到現在這個物件 (詳情可以看下方reference連結). 如下範例, 不管哪種方式parameter傳入都會建立一個temp的string物件然後constructor結束再把它destruct掉, 但initializer list直接拿這個temp物件呼叫t的copy constructor來Init Test::t, 但assignment operator就是要先call default constructor Test::t來建立物件,接著再assignment oprator把t等於x, 等於多呼叫了一個default constructor
...
初始化順序跟class member宣告順序有關 而不是跟initialized list順序 如下:<br />
初始化順序仍是先init a,在init b
...
所以以下寫法就會導致跟預期不同的結果, a不會是 x*x, 而是根據stack上b的值來決定a的值

```
class Test {
  int a, b;
public:
Test(int x,int y) : b(x), a(b * b);
}
```


Reference:<br />
- [geeks for geek](https://www.geeksforgeeks.org/when-do-we-use-initializer-list-in-c/)
...
char*, std::string都會自動在最後加\0, 記錄這是這個字串的最後。

```
char *t = "hello" -> hello這個字串放在data segment (read only), t pointer point到這個地方
char t[] = "hello" -> 一個t pointer指向stack上面某個位置，把"hello"放到stack這個地方上
```
...
以下三種casting(static, reinterpret, const)是c++獨有的syntax, C也能做到同樣的事，只是這些事語法糖, 除了能夠避免coding的錯誤外，這三種casting也能在code search時協助search相關的symbol, 一般的C的casting, 會在code search時沒辦法link, 導致很多casting時很難trace code。且這三種casting會多更多compile time check也讓code更readable

#### Type punning

C/C++是一個有data type的語言，但因為可以操控記憶體的緣故，我們可以透過casting來reinterpret這塊記憶體的data type, type punning就是透個取得一個變數的記憶體位址，然後把他cast成某個data type, 用星號dereference來得到這個轉變成新的data type後的值

#### Static Casting

在靜態狀況下作casting, 會做一些compile time check

```
double a = static_cast<another_class>(b);

char c;
int *p = static_cast<int *>(&c);  // compile failed, invalid
```

##### reinterpret cast

相當於type punning的模式但更readable

```
double a = reinterpret_cast<another_class *>(&A);
```
...
特別用於class繼承,多形的casting部分, 可以用來檢查現在這個cast是否轉換成功。繼承中失敗原因通常在於一個base class下有兩個derived class, 當想要從一個derived class cast到另一個時會出事，因為雖然都繼承自base class, 但他們有各自的function, attirbute, 很容易導致不預期得錯誤，單純指標指向另一個class instance 很難debug或發現，所以可以用dynamic cast來測試，這種狀況就會return null

```
Base *b = derived;
another_class A = dynamic_class<another_class *>b;

if(dynamic_cast<another_class *>(b) == another_class){...} // 像是java instaceof的用法
```

會檢查是否能這樣cast, 如果不行, dynamic class會回傳NULL, 所以可以透過檢查lhs parameter來看dynamic class是否成功

### move semantics

問題：<br />
對於C++ function pass by value or return by value會construct新的object然後return, 但如果傳入的是暫時物件，其實沒必要做copy by value, 只要把pointer轉指向這物件即可
...
在沒有move semantics的狀況下，這樣一個單純const char* 轉成std::string會有兩次的記憶體複製，讓整件事沒效率，一次是std::string("name")把const char建立成一個std::string，一個是copy constructor，把第一步建立的std::string在複製到a裡面，所以總共兩次的memcpy，但第二次其實並沒有必要，只要把pointer指向第一次建立的物件，第一次的物件pointer改成指向null即可。就可以減少一次記憶體複製。基本上對於對於一個暫時建立的物件，複製只要用move, 不用真的construct。

第二個例子中，string "A"一樣先呼叫string(const char*)建立物件，接著在呼叫copy constructor string(string &a)把物件傳入func, 但其實第二個copy constructor部分只要把pointer指向這物件，並把第一個constructor建立的物件pointer設成null即可。

```
string(string &&a)  -> move constructor, 接收一個rvalue的string

string b = "test";
string test = std::move(b) -> move把string轉成rvalue, 不然單純string test = b 會呼叫string(string &a) 這個copy constructor 而不是string( &&a) 這個move constructor
```

#### assign operator and constructor

寫法上很相似，不能單純用 "="來分辨，要看是否建立物件

```
string a = b (copy constructor, 因為建立新物件)

string c;
c = b (assign operator, 相當於c.assign(b))
```
#### static

static有五種用法

##### class內的variable/method宣告static

代表這個method, variable不屬於此class, 也就是不用建立這個class的object就可以使用該method, variable, 但static method不能存取class內其他non-static variable, class內的static variable要在class外面做init, 不然不會過 (int A::staticvar = 1)

##### 檔案內的function, variable宣告static

代表該function, variable的scope就是在這個檔案內, 其他檔案並沒有辦法呼叫到這個function, variable, 反之如果function, variable沒有加static, 其他檔案就可以透過呼叫extern來使用這個檔案內的這些function, 變數
