...
或許也可以先選定起點, 把跟起點相鄰的edge push進heap李, 每次pop heap中最小的直出來 看能不能連到新點, 可以的話他就是MST其中一個edge, 然後再把新連到的點的外接的edge也都Push進heap裡. 然後就這樣不斷加新的邊, 複雜度為O(VlogE), 每次edge的pop是Log(E),然後我們要連接V個點,所以大約做個V次, 空間複雜度為O(E),
...
用heap配合map(map用來迅速定位某個node再heap哪裡): O(Elog(V)) (做decrease key E次, 每次logv,因為改值後要做up,down heap, O(ElogV)), Space complexity O(E), worst case時每次decrease key都要push node進priority_queue.
