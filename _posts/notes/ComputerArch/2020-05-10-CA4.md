...
如圖, 我們把一個城市的記憶體拆成6 pages, 把他分別對應到physical memory不同的地方(page也可以對應到disk, 不一定要在main memory)

![](/assets/images/notes/CA/4-11.png)

只是這種方式, 因為程式的memory到physical memory已經不是連續了, 所以不難單純用來做某種平移, 必須要用page table來找到virtual address跟對應的phyisical address, 詳情可以見virtual address的筆記
