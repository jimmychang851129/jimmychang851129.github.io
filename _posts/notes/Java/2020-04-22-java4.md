...
- private data雖然會被繼承，**但是不能直接呼叫、get,set data或透過derived classs的method get/set**，一定只有base class的method能修改存取這些被繼承的private data(詳情可見oopjudge/test/A,B,AB.java)
...
package access = default access = friendly access
宣告變數、method時會在前面放public/private/protected，但沒放的話會是default access type，只有在同個package底下的class能access
**default package**:同個資料夾下的其他class
1. package access在同個package底下的class能隨意存取變數、method
2. 在不同package下，就算是繼承關係，class就不能存取該變數
